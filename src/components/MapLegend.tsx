...
export default function MapLegend({ mapType }: Props) {
  ...
  return (
    <div className="absolute top-4 right-4 z-1000 w-52 rounded-sm shadow-sm p-2 bg-background/50 border border-accent/70 flex flex-col gap-2">
      <h3 className="text-xs font-semibold text-foreground">{data.title}</h3>
      <div
        className="w-full h-3 rounded-xl border border-accent/70"
        style={{
